...
/// <summary>
/// 具体状态：步枪（Rifle）× 正常手部（Normal）× 滞空（Jumping）。
/// 与空手滞空同款（test.cs 滞空分支）：
/// - 水平：起跳瞬间【捕获】上一状态（地面）写入的速度向量作为跳跃移动速度——
///   滞空期间方向与大小均保持不变（纯惯性，不做输入操控 / 加速度插值）：
///   站立跳继承 0 → 纯向上；行走/瞄准中跳出继承当前速度 → 带前冲；
/// - 垂直：手写重力累积；起跳初速度在 Enter 反推 v = √(2·|g|·h)；
/// - 落地：垂直速度 ≤ 落地判定阈值且物理着地时【提议】回（Rifle, Normal, Ground）（请求边裁决）；
/// - 位移：OnAnimatorMove 全量接管——跳跃动画没有水平根运动，水平位移由脚本按继承速度接管，
///   垂直由手写重力接管；
/// - 动画参数：本类只写水平/垂直速度分量；body posture / hand posture / player handing
///   由主体类按当前状态组合每帧同步（本状态 Hand=Normal → 瞄准取消，动画回到步枪正常分支）。
/// 进入方式：步枪地面跳（信号边）/ 瞄准中跳（信号边，取消瞄准）/ 瞄准中走出边沿（请求边）。
/// </summary>
public class Rifle_Normal_Jumping_State : PlayerStateBase
{
    ...
    public override void Enter(PlayerContext ctx)
    {
        ...
        // ② 瞄准中跳出 / 走出边沿（信号边/请求边）：保留已累积的下落速度，只捕获水平分量。
    ...
    public override void Tick(PlayerContext ctx)
    {
        ...
        {
            ctx.RequestTransition(PlayerHanding.Rifle, PlayerHandPosture.Normal, PlayerBodyPosture.Ground);
            return;
        }
