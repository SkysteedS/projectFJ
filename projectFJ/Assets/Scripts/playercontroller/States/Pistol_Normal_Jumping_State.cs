...
/// <summary>
/// 具体状态：手枪（Pistol）× 正常手部（Normal）× 滞空（Jumping）。
/// 复刻自 Rifle_Normal_Jumping_State（原样复刻，无特殊效果）：
...
/// - 落地：垂直速度 ≤ 落地判定阈值且物理着地时【提议】回（Pistol, Normal, Ground）（请求边裁决）；
/// - 位移：OnAnimatorMove 全量接管——跳跃动画没有水平根运动，水平位移由脚本按继承速度接管，
///   垂直由手写重力接管；
/// - 动画参数：本类只写水平/垂直速度分量；body posture / hand posture / player handing
///   由主体类按当前状态组合每帧同步。
/// 进入方式：手枪地面跳（Jump 信号边）/ 手枪地面走出边沿（请求边）。
/// </summary>
public class Pistol_Normal_Jumping_State : PlayerStateBase
{
    ...
    public override void Tick(PlayerContext ctx)
    {
        ...
        {
            ctx.RequestTransition(PlayerHanding.Pistol, PlayerHandPosture.Normal, PlayerBodyPosture.Ground);
            return;
        }
